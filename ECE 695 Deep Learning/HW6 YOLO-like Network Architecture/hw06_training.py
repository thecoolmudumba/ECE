# -*- coding: utf-8 -*-
"""
Created on Sat Apr 10 15:01:37 2021
    ...
    def set_dataloaders(self, train=False, test=False):
        ...
        if test:
            ...
            self.test_dataloader = torch.utils.data.DataLoader(dataserver_test, 
                                                 self.rpg.batch_size, shuffle=False)
...
#                                    if yolo_debug:
                    ...
                    print(f'bbox actual label {classes[bbox_label_tensor[0][0]], classes[bbox_label_tensor[0][1]], classes[bbox_label_tensor[0][2]]}')
                    if len(objects_detected) == 3:
                        print(f'bbox predicted label {objects_detected[0], objects_detected[1], objects_detected[2]}')
                    
                        if classes[bbox_label_tensor[0][0]] == objects_detected[0] and classes[bbox_label_tensor[0][1]] == objects_detected[1]:
                            print("Accuracy is 100 Percent")
                        elif classes[bbox_label_tensor[0][0]] == objects_detected[0] or classes[bbox_label_tensor[0][1]] == objects_detected[1]:
                            print("Accuracy is 50 percent")
                        else:
                            print("Accuracy is 0 percent")
                        
                        # if classes[bbox_label_tensor[0][0]] == objects_detected[1] and classes[bbox_label_tensor[0][1]] == objects_detected[0]:
                        #     print("Accuracy is 100 Percent")
                        # elif classes[bbox_label_tensor[0][0]] == objects_detected[1] or classes[bbox_label_tensor[0][1]] == objects_detected[0]:
                        #     print("Accuracy is 50 percent")
                        # else:
                        #     print("Accuracy is 0 percent")
                    ...
                    if display_images:
                        ...
                        for t in range(3):
                            ...
                            # put a label next to the bounding box
                                image = cv2.putText(image, objects_detected[t], (int(bbox_pred_center[t][0]+10),int(bbox_pred_center[t][1]+10)), cv2.FONT_HERSHEY_SIMPLEX ,0.3, (0 , 0 ,255), 1)
                                image = cv2.putText(image, classes[bbox_label_tensor[0][t]], (int(bbox_tensor[0][t][0]+10),int(bbox_tensor[0][t][1]+10)), cv2.FONT_HERSHEY_SIMPLEX ,0.3, (0, 255, 0), 1)
