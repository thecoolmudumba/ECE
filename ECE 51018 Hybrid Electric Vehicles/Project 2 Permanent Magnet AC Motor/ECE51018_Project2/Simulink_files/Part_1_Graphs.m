...
for i = 1:N_w
    ...
    if V_p(i) < V_batt/sqrt(3)
        ...
        Pe(i) = (3/2)*(V_qs(i)*I_qs(i) + V_ds(i)*I_ds(i));
    else
        % because V_peak cannot exceed V_batt/sqrt(3), we will arrive a
        % point where increasing w_r requires us to violate V_peak. In
        % order to not violate this constraint, we need to make I_ds
        % negative such that V_qs decreases, and hence V_peak decreases.
        % The decrease in V_peak corresponds to decrease in I_qs. If I_qs
        % decreases, so does maximum torque.
        N_i = 7000;
        ...
        for j = 1:N_i
            ...
            if V_p_temp(j) < V_batt/sqrt(3)
                ...
                Pe_temp(j) = (3/2)*(V_qs_temp(j)*I_qs_temp(j) + V_ds_temp(j)*I_ds_temp(j));
            end
        end
...
w_rm = (2/P)*w_r;
w_rm_rpm = w_rm*(60)/(2*pi);

figure(1)
plot(w_rm_rpm, Te, 'LineWidth',1)
ylim([0,130])
title('Maximum Torque versus Mechanical Rotational Speed Curve')
xlabel('Mechanical Rotational Speed (RPM)')
ylabel('Torque (N-m)')

figure(2)
plot(w_rm_rpm, Te.*w_rm/1000)
title('Maximum Mechanical Power versus Mechanical Rotational Speed Curve')
xlabel('Mechanical Rotational Speed (RPM)')
ylabel('Power (kW)')

figure(3)
plot(w_rm_rpm, I_ds)
title('I_{ds}^r current needed for Maximum Torque')
xlabel('Mechanical Rotational Speed (RPM)')
ylabel('I_{ds}^r (A)')
ylim([-250 250])
